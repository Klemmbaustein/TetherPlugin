...
#define PLUGIN_NAME "TetherInstaller"
// all northstar log tags use 9 characters. Keep the same length to make your logs look a bit nicer
#define PLUGIN_LOG_NAME "TETHER_MM" 
...
EXPORT const char* GetString(IPluginId* self, PluginString prop)
{
	switch (prop) {
	case ID_NAME:
		return PLUGIN_NAME;
...
EXPORT int64_t GetField(IPluginId* self, PluginField prop)
{
	switch (prop) {
	case ID_CONTEXT:
